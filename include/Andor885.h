...
class Andor885:public Camera{
public:

	
	Andor885(Trigger* t,int triggerLine, int deviceNum=0,int orientation=0);
	static int init();
	long camHandle;
	~Andor885();//close and deinitialize Andor885
...
//start actual implementation
	...
	bool isPresent;
	float temp; //temperature to cool the Andor885 down to
	float exp;
	int gain;
...
private:
	...
	void coolDown();//switch cooler on and set Andor885 temperature to global temp variable
