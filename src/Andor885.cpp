...
#define Andor885SelCam(functionCall,camPtr) WaitForSingleObject(AndorSelectCamMutex, INFINITE);\
											camPtr->getError(SetCurrentCamera(camPtr->camHandle));\
											camPtr->getError(functionCall);\
											ReleaseMutex(AndorSelectCamMutex);


void Andor885::getError(unsigned int err_val){
	//system("pause");
	if (err_val!=DRV_SUCCESS){
		logFile.write(string("Andor885 Driver Error Code: ")+::toString((int)err_val), true);
		int stat;
		GetStatus(&stat);
		logFile.write(string("Andor885 Camera ")+toString(this->camHandle)+" status code:"+toString(stat),true);
		ReleaseMutex(AndorSelectCamMutex);
...
struct Series{
	...
	Scan* scan;
	int camNum;
	Series(Andor885* a,int numImages,int seriesNum,string tempDir,Scan* scan, int camNum):a(a),numImages(numImages),seriesNum(seriesNum),tempDir(tempDir),scan(scan),camNum(camNum){}
};



//abstract methods implementation
HANDLE Andor885::startSeries(int numImages,int seriesNum, Scan* scan, int camNum){
	unsigned int err1;
	waitIdle();
	static int dirNum=-1;
	...
	dirNum++;
	string tempDir=string(ANDOR885TEMPDIR)+::toString(dirNum,length)+"\\";
...
HANDLE Andor885::startSeries2(int numImages,AcquisitionParameters ap,int seriesNum,Scan *s){
	waitIdle();
...
double Andor885::startFocusSeries(int num,AcquisitionParameters ap){
	unsigned threadID;
	double ret;
		this->getImageRegion(ap,minXPixel,minYPixel,maxXPixel,maxYPixel);
...
double Andor885::startStepwiseFocusSeries(int num, AcquisitionParameters ap){
	unsigned threadID;
	waitIdle();
...
void Andor885::terminateSeries(){
	CheckExists()
	Andor885SelCam(AbortAcquisition(),this);
}
...
void Andor885::takeAccumulation(AcquisitionChannel* ac, int num, string fileName){
	Accum a;
	a.fileName=fileName;
	a.num=num;
	a.c=this;
...
unsigned __stdcall Andor885::SaveAccum(void* param){
	Accum* a=(Accum*) param;
	long acc,series;
	for(int i=0;i<a->num;i++){
		Andor885SelCam(GetAcquisitionProgress(&acc,&series),a->c)
		if (!a->c->isIdle())
...
LONGLONG Andor885::takePicture(AcquisitionChannel* ac, string fileName){
	LONGLONG ret=0;
	static int temp=0;
	if (fileName=="temp"){
		temp++;
		fileName=string(ANDOR885TEMPDIR)+fileName+::toString(temp,5);
	}
	...
	if (c.chan==ac->chan && c.m==ac->m){//make sure we will not over-expose
		...
		if (guessMax>ANDOR885MAXPIXELVAL*0.95) {
			logFile.write("ANDOR: Image will likely over-expose. Exp:"+toString(ac->ap.exp)+" Gain"+toString(ac->ap.exp),DEBUGANDOR885);
		}
	}
	c=*ac;
	this->getImageRegion(ac->ap,minXPixel,minYPixel,maxXPixel,maxYPixel);
	...
	if (pic->max()>ANDOR885MAXPIXELVAL*0.95) logFile.write("Camera: Warning. Picture taken was overexposed or close to saturation. Please adjust settings",DEBUGANDOR885);
	max=pic->max();
	if (fileName.at(0)!='!') {
		...
		getScore(pic,&ret);
		if (ac->showOnScreen) Andor885::showImage(pic,fileName+string(" Score=")+::toString(ret));
	}	
	delete pic;
	return ret;
}
...
void Andor885::adjustIntensity(AcquisitionChannel& ac,double percentSaturation){
	CheckExists()
	bool redo=false;
	unsigned short max;
	this->takePicture(ac.ap.exp,ac.ap.gain,ac.ap.bin);
	...
	CImg<unsigned short>* pic=getNewPic();
	max=pic->max();
	if (max>ANDOR885MAXPIXELVAL) {
		logFile.write("Camera: Adjustment error: Picture taken was overexposed. adjusting settings and retrying",DEBUGANDOR885);
		max=((double)ANDOR885MAXPIXELVAL)*percentSaturation/0.5;//artificially adjust max so we get half saturation
		redo=true;
	}
	delete pic;
	...
	int newGain=ac.ap.gain*ANDOR885MAXPIXELVAL*percentSaturation/max;
	if (newGain>32){
		newGain=32;
	}if (newGain<3){
		newGain=3;
	}
	newExp=((double)ANDOR885MAXPIXELVAL)*percentSaturation*ac.ap.exp/(((double)max)*(((double)newGain)/ac.ap.gain));
	if (newGain!=ac.ap.gain){
		logFile.write(string("Camera (AdjustIntensity) Max pixel value was:")+toString(max)+" out of "+toString(ANDOR885MAXPIXELVAL)+" Desired Percent Saturation is "+toString(percentSaturation)+" Changed gain from: " +toString(ac.ap.gain)+" to "+toString(newGain));
	}
	if (newExp!=ac.ap.exp){//most likely it was changed
		logFile.write(string("Camera (AdjustIntensity) Max pixel value was:")+toString(max)+ " out of "+toString(ANDOR885MAXPIXELVAL)+" Desired Percent Saturation is "+toString(percentSaturation)+" Changed exposure from: "+toString(ac.ap.exp)+" to "+toString(newExp));
	}
	ac.ap.exp=newExp;
	ac.ap.setGain(newGain);
	if (redo) adjustIntensity(ac,percentSaturation);
}
void Andor885::startLiveView(LiveViewData lvd){
	CheckExists()
	if (live)
		stopLiveView();
	unsigned threadID;
...
void Andor885::waitLiveViewStart(){
	CheckExists()
...
void Andor885::saveLiveImage(string fileName){
	this->fileName=fileName;
	this->takepic=true;
}

void Andor885::stopLiveView(){
	CheckExists()
	if (!live)
		return;
	_isLiveView=false;
...
double Andor885::getImageWidth(AcquisitionParameters& ap){
	int minX,minY,maxX,maxY;
	getImageRegion(ap,minX,minY,maxX,maxY);
	return (maxX-minX+1)*getPixelSize();
}

double Andor885::getImageHeight(AcquisitionParameters& ap){
	int minX,minY,maxX,maxY; 
	getImageRegion(ap,minX,minY,maxX,maxY);
...
bool Andor885::isValid(AcquisitionParameters &ap){
	return true;
}

double Andor885::getTemp(){
	return this->getCoolTemp();
}

unsigned __stdcall Andor885::dat2Tiff(void* param){
	...
	for(int i=1;i<s->numImages+1;i++){
		...
		while(true){//wait for file to exist
			if (WAIT_OBJECT_0==WaitForSingleObject(s->a->getAbortEvent(s->scan),0)){
				Andor885SelCam(AbortAcquisition(),s->a);
...
Andor885::Andor885(Trigger* t,int triggerLine,int deviceNum,int orientation):
...
int Andor885::init(){
	...
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
		//cout<<"could not initialize Andor885 driver: "<<err_val<<endl;
		cout<<"Andor SDK driver cannot be initialized"<<endl;
		return 0;
	}
	return 1;
}
unsigned __stdcall Andor885::setupThread(void* param){
	Andor885* a=(Andor885*) param;
	...
	*/
	if (!a->t || !a->t->isValidLine(a->triggerLines.front())){
		logFile.write("Andor885 NOT PRESENT....needs valid trigger line",true);
		a->isPresent=false;
		return 0;
	}
	...
	if (DRV_SUCCESS!=err){//look in current directory for .ini file
		...
		logFile.write("Andor885 NOT PRESENT",true);
		return 0;
	}
	...
	if (DRV_SUCCESS!=err){//look in current directory for .ini file
		//cout<<"could not initialize Andor885 driver: "<<err_val<<endl;
		a->isPresent=false;
		cout<<"Andor885 NOT PRESENT"<<endl;
		return 0;
	}
	...
	Andor885SelCam(err_val=Initialize(NULL),a);
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
	...
	Andor885SelCam(SetCoolerMode(1),a)//temperature is maintained on shutdown Remember to set temperature to 20C
	Andor885SelCam(err_val=GetDetector(&a->XPixels,&a->YPixels),a)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
	...
	Andor885SelCam(err_val=SetShutter(0,1,0,0),a)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
		cout<<"could not set shutter timings: "<<err_val<<endl;
		system("pause");
		exit(0);
	}
	Andor885SelCam(err_val=SetReadMode(4),a)//image acquisition mode
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
		cout<<"could not set read mode to image acquisition: "<<err_val<<endl;
		system("pause");
		exit(0);;
	}
	Andor885SelCam(err_val=SetVSSpeed(3),a)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
	...
	float speed;
	Andor885SelCam(err_val=GetVSSpeed(3,&speed),a)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
	...
	Andor885SelCam(err_val=SetVSAmplitude(0),a)//can be 0-4 with 4 being the highest voltage for high readout speeds
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
		cout<<"could not set VS Amplitude: "<<err_val<<endl;
		system("pause");
		exit(0);;
	}
	Andor885SelCam(err_val=SetHSSpeed(0,0),a)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
		cout<<"could not set HSSpeed: "<<err_val<<endl;
		system("pause");
		exit(0);;
	}
	Andor885SelCam(err_val=GetHSSpeed(0,0,0,&speed),a)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
	...
	int numADChan;
	Andor885SelCam(err_val=GetNumberADChannels(&numADChan),a)
	int chan=0;
	//cout<<"Number of AD Channels is: "<<numADChan<<" We selected channel "<<chan<<endl;
	
	Andor885SelCam(err_val=SetADChannel(chan),a)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
	...
	int numAmp;
	Andor885SelCam(err_val=GetNumberAmp(&numAmp),a)
	//cout<<"Number of Output Amp Channels is: "<<numAmp<<endl;
	int filterMode;
	Andor885SelCam(GetFilterMode(&filterMode),a)
	if (filterMode==0){
		//cout<<"cosmic ray filter is OFF"<<endl;
	}else{
		//cout<<"cosmic ray filter is ON"<<endl;
	}
	Andor885SelCam(err_val=SetAcquisitionMode(1),a)//single scan mode...5 would be run til abort
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
		cout<<"could not set acquisition mode to single scan: "<<err_val<<endl;
		system("pause");
		exit(0);;
	}
	Andor885SelCam(err_val=SetFrameTransferMode(0),a)//frame transfer mode OFF
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
		cout<<"could not set Andor885 to FT Mode: "<<err_val<<endl;
		system("pause");
		exit(0);;
	}

	Andor885SelCam(err_val=SetTriggerMode(0),a)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
		cout<<"could not set internal triggering mode: "<<err_val<<endl;
		system("pause");
		exit(0);;
	}
	Andor885SelCam(SetOutputAmplifier(0),a)//standard emmccd gain
	int noGains;
	Andor885SelCam(err_val=GetNumberPreAmpGains(&noGains),a)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
	...
	Andor885SelCam(GetPreAmpGain(0,&pgain0),a)
	//cout<<"Number of PreAmp Gains: "<<noGains;
	//cout<<" Gain 0="<<pgain0<<"x";
	Andor885SelCam(GetPreAmpGain(1,&pgain1),a)
	//cout<<" Gain 1="<<pgain1<<"x";
	Andor885SelCam(GetPreAmpGain(2,&pgain2),a)
	//cout<<" Gain 2="<<pgain2<<"x"<<endl;
	//cout<<" Setting PreAmp Gain to "<<pgain0<<"x"<<endl;
	Andor885SelCam(err_val=SetPreAmpGain(2),a)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
	...
	//SetKineticCycleTime();
	long numPics;
	Andor885SelCam(SetDMAParameters(1,.01),a)
	Andor885SelCam(err_val=GetSizeOfCircularBuffer(&numPics),a)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
	...
	Andor885SelCam(err_val=SetDriverEvent(a->newImage),a)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
		cout<<"could not set driver event: "<<err_val<<endl;
		system("pause");
		exit(0);;
	}
	Andor885SelCam(err_val=SetBaselineClamp(1),a)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
	...
	a->coolDown();
//	Andor885SelCam(SetEMAdvanced(1),this)//this allows for higher gain values but do we need it?
//	Andor885SelCam(SetEMGainMode(3),this)
//	Andor885SelCam(GetEMGainRange(&minGain,&maxGain),this)
	Andor885SelCam(SetEMAdvanced(0),a)//this limits em gain to 300 which should be fine
	Andor885SelCam(SetEMGainMode(2),a)
	Andor885SelCam(GetEMGainRange(&a->minGain,&a->maxGain),a)

	Andor885SelCam(GetBitDepth(chan, &a->bitDepth),a)
	Andor885SelCam(SetEMGainMode(2),a)//Real EM Gain is 3...note to turn EM off we need to be in mode 0 or 1 and send a DAC value of 0
	//Linear gain is 2
	//12bit gain is 1
	//8bit gain is 0
	//setExposure(.1);
	...
	if (!DeleteDirectory(string(ANDOR885TEMPDIR).c_str()))
...
void Andor885::closeAll(){
	CheckExists()
//	Disp::closeAll();
}

long Andor885::spoolProgress(){
	CheckExists(0)
	long ret;
	unsigned int err_val;
	Andor885SelCam(err_val=GetSpoolProgress(&ret),this)
		if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
...
void Andor885::waitIdle(){
	CheckExists()
	while(!isIdle());
}

void Andor885::coolDown(){
	CheckExists()
	int minT,maxT;
	Andor885SelCam(GetTemperatureRange(&minT,&maxT),this)
	//cout<<"Min Temp for cooling is: "<<minT<<" Max Temp for cooling is: "<<maxT<<endl;
	if (temp<minT || temp>maxT){
		cout<<"error Andor885 cooling temperature is out of range"<<endl;
		system("pause");
		exit(0);;
	}
	Andor885SelCam(SetTemperature((int)temp),this)
	Andor885SelCam(CoolerON(),this)
	//while(GetTemperature(&temp)!=DRV_TEMP_STABILIZED){}
	//cout<< "Andor885 temperature has reached "<<temp<<" degrees Celsius"<<endl;
}

void Andor885::heatUp(){
	CheckExists()
	Andor885SelCam(SetTemperature(20),this)//Actively return to ambient
	Andor885SelCam(CoolerOFF(),this)//not sure if this call is necessary
	unsigned int err_val;
	Andor885SelCam(err_val=GetTemperatureF(&temp),this)
	if (err_val!=DRV_TEMP_OFF)
	logFile.write(string("Error: Andor885 could not get camera temperature: ")+toString(int(err_val)),true);
	logFile.write(string("Current temp is ")+toString(temp)+" degree C. Let camera heat back up to ambient.",DEBUGANDOR885);
}

float Andor885::getCoolTemp(){
	CheckExists(0)
	waitIdle();
	float target, ambient, coolvolt;
	unsigned int err_val;
	Andor885SelCam(err_val=GetTemperatureStatus(&temp, &target, &ambient, &coolvolt),this)
	if (err_val!=DRV_SUCCESS)
...
void Andor885::waitTemp(){
	CheckExists()
	cout<<"Waiting for Andor885 to cool down to "<<this->temp<<" deg C..."<<endl;
	int temp;
	Timer t;
	t.startTimer();
	int i=0;
	unsigned int b;
	while(true){
		Andor885SelCam(b=GetTemperature(&temp),this)
	...
	cout<<"...Andor885 Temperature Reached!"<<endl;
}

long Andor885::totalNum(){
	CheckExists(0)
	long ret;
	Andor885SelCam(GetTotalNumberImagesAcquired(&ret),this)
	return ret;
}

int Andor885::getBitDepth(){
	return bitDepth;
}

float Andor885::getPixelSize(){
	CheckExists(0)
	static bool once=false;
	float y;
	static float x;
	if (once) return x;
	Andor885SelCam(GetPixelSize(&x,&y),this)
	once=true;
	assert(x==y);
	return x;
}
void Andor885::saveTiff(string fileName, CImg<unsigned short>* c,string descr, bool asEightBit){
	fileName+=".tif";
	if (asEightBit){
		c->cut(0,ANDOR885MAXPIXELVAL);
		c->operator>>=(ANDOR885BITS-8*sizeof(unsigned char));//multiply by 255/16383
...
void Andor885::saveTiff(CImg<unsigned short>* c,ImageProperties& ip){
	...
	if (ip.ac->showOnScreen) Andor885::showImage(c,ip.name+string("MaxVal=")+::toString((int)ip.max)+"/"+::toString(ANDOR885MAXPIXELVAL)+" "+ip.ac->toString());
	...
	TIFFRewriteDirectory(tif);
	TIFFClose(tif);
	logFile.write(fileName+" saved",DEBUGANDOR885);
}

void Andor885::showFile(string fileName){
...
void Andor885::showImage(CImg<unsigned short>* Cimage, string title){
//	CheckExists()
	Arg* a= new Arg();
//	ostringstream myStream;
//	//myStream.str("");
...
unsigned __stdcall Andor885::ButtonWaitThread(void *param){
...
void Andor885::testMultipleCameras(){
...
void Andor885::testStop(){
	SetCurrentCamera(camHandle);
	AbortAcquisition();
}

CImg<unsigned short>* Andor885::testGetPic(){
...
unsigned __stdcall Andor885::LiveThread(void *param){
	...
	Andor885SelCam(SetTriggerMode(0),c)//internal trigger
	Andor885SelCam(err_val=SetAcquisitionMode(5),c)//run til abort
	Andor885SelCam(SetReadMode(4),c)
	Andor885SelCam(SetKineticCycleTime(0),c)
	Andor885SelCam(SetFrameTransferMode(1),c)
	...
	c->setGain(gain);
	Andor885SelCam(err_val=SetSpool(0,2,"",10),c)
	float kin,acc;
	//Timer::wait(100); BUG IN ANDOR DRIVER THIS MAY BE REQUIRED
	Andor885SelCam(GetAcquisitionTimings(&(c->exp),&acc,&kin),c)
	if (c->exp>desiredExp) {
		Andor885SelCam(SetFrameTransferMode(0),c)
		c->setExposure(desiredExp);
	}

	//Andor885SelCam(FreeInternalMemory(),c) not really necessary?
	//Andor885SelCam(PrepareAcquisition(),c) not really necessary?
	...
	*lva->lvd.zoom=getZoom(*lva->lvd.scaleFactor);;//total zoom for current view;
	...
	int stat;
	Andor885SelCam(err_val=StartAcquisition(),c)
	//Timer::wait(5000);
	Andor885SelCam(GetStatus(&stat),c)
	...
	Andor885SelCam(err_val=GetMostRecentImage16(copyImage.data(),numXpixels*numYpixels),c)
	...
	while(c->_isLiveView){
		...
		if (ret==WAIT_OBJECT_0+1){//new image available
			...
			if (c->takepic){
				Andor885::saveTiff(c->fileName,&copyImage);
				Andor885::saveTiff(c->fileName+"scaleBar",&displayImage);
				c->takepic=false;
			}
	...
	Andor885SelCam(err_val=AbortAcquisition(),c)
	Andor885SelCam(err_val=SetAcquisitionMode(1),c)//single scan
...
unsigned __stdcall Andor885::FileThread(void *param){
...
unsigned __stdcall Andor885::ImageThread(void *param){
...
void Andor885::setExposure(float newExp){
	CheckExists()
	waitIdle();
	this->exp=newExp;
	unsigned int err_val;
	Andor885SelCam(err_val=SetExposureTime(this->exp),this)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
...
float Andor885::getExposure(){
	CheckExists(0)
	return exp;
}

int Andor885::getGain(){
	CheckExists(0)
	return gain;
}


void Andor885::setGain(int& newGain){
	CheckExists()
	waitIdle();
	...
	if (newGain<minGain){
		...
		Andor885SelCam(SetEMGainMode(0),this)
		Andor885SelCam(SetEMCCDGain(0),this)//gain=minGain;
		newGain=0;//this should update the current channel if we have access to it
	}else {
		if (gain==0)
			Andor885SelCam(SetEMGainMode(2),this)
		if (newGain>maxGain){	
		...
		Andor885SelCam(SetEMCCDGain(newGain),this)
	}
	gain=newGain;
}
...
void Andor885::setBin(int newBin){
	CheckExists()
	waitIdle();
	...
	*/
	...
	unsigned int err_val;
	Andor885SelCam(err_val=SetImage(bin,bin,minXPixel+1,maxXPixel+1,minYPixel+1,maxYPixel+1),this)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
...
void Andor885::getNumPixels(const AcquisitionParameters& ap,int& numXpixels,int& numYpixels){
	numXpixels=1;
	numYpixels=1;
	CheckExists()
	int minX,minY,maxX,maxY;
...
void Andor885::getImageRegion(const AcquisitionParameters& ap,int& minX,int& minY,int& maxX,int& maxY){
	CheckExists()
...
void Andor885::setImageRegion(int opt, int percentX, int percentY){
	CheckExists()
	int tminXPixel,tminYPixel,tmaxXPixel,tmaxYPixel;
	int ANDORHACK=1;//this min X pixel to avoid nasty stuff in image  could just be the loaner Andor885
	switch(opt){
		case 0:
...
void Andor885::takePicture(float newExp, int newGain, int newBin){
	CheckExists()
	waitIdle();
	...
	ResetEvent(this->newImage);
	if (newExp<0.35){
	Andor885SelCam(SetFrameTransferMode(0),this)//non-frame transfer mode
	}else {
		Andor885SelCam(SetFrameTransferMode(1),this)
	}
	Andor885SelCam(SetAcquisitionMode(3),this)//kinetics
	Andor885SelCam(SetTriggerMode(6),this)//external start trigger
	Andor885SelCam(SetNumberKinetics(1),this)//single snap
	Andor885SelCam(SetKineticCycleTime(0),this)
	float acc,kin;
	Andor885SelCam(GetAcquisitionTimings(&exp,&acc,&kin),this)
	logFile.write(string("Snap. Actual exposure: ")+::toString(exp));
	Andor885SelCam(PrepareAcquisition(),this)
	Andor885SelCam(StartAcquisition(),this)
	Timer::wait(200);
	//waitIdle();//we should not need to do anything during this period
	
}

bool Andor885::isIdle(){
	CheckExists(true)
	int stat;
	Andor885SelCam(GetStatus(&stat),this)
...
LONGLONG Andor885::Kinetics(){
	CheckExists(0)
	LARGE_INTEGER realstart,start,end,freq;
	QueryPerformanceFrequency(&freq);
	Andor885SelCam(SetTriggerMode(6),this)
	Andor885SelCam(SetAcquisitionMode(3),this)//kinetics series
	Andor885SelCam(SetNumberKinetics(100),this)//4 pictures
	Andor885SelCam(SetExposureTime(.1),this)//exposure time =20us
	Andor885SelCam(SetKineticCycleTime(0),this)//40ms cycle time
	Andor885SelCam(SetFrameTransferMode(1),this)//non-frame-transfer mode
	float Camexp,acc,kin;
	Andor885SelCam(GetAcquisitionTimings(&Camexp,&acc,&kin),this)
	...
	QueryPerformanceCounter(&start);
	unsigned int err_val;
	Andor885SelCam(err_val=StartAcquisition(),this)
	QueryPerformanceCounter(&end);
	...
	Andor885SelCam(SetAcquisitionMode(1),this)
	return end.QuadPart-realstart.QuadPart;
}

bool Andor885::isTriggerable(AcquisitionParameters &ap){
	CheckExists(true)
	unsigned int err_val;
	if (!isValid(ap)) return false;
	this->bin=ap.bin;
	this->getImageRegion(ap,minXPixel,minYPixel,maxXPixel,maxYPixel);
	Andor885SelCam(err_val=SetTriggerMode(1),this)//external trigger
	Andor885SelCam(err_val=SetAcquisitionMode(3),this)//kinetics
	Andor885SelCam(err_val=SetReadMode(4),this)//image acquisition mode
	Andor885SelCam(err_val=SetNumberKinetics(10),this)//number of pictures to acquire
	Andor885SelCam(err_val=SetExposureTime(0),this)//delay before exposure
	setBin(ap.bin);
	setGain(ap.gain);
	Andor885SelCam(err_val=SetKineticCycleTime(0),this)//we want minimum repition rate
	Andor885SelCam(err_val=SetFrameTransferMode(1),this)//frame transfer mode
	char* dir=toCharStar(string(DEFAULTWORKINGDIR)+"Error-this should not exist");
	Andor885SelCam(err_val=SetSpool(1,2,dir,10),this)
	float acc,exp2,minTrigTime;;
	Andor885SelCam(GetAcquisitionTimings(&exp2,&acc,&minTrigTime),this)
	delete[] dir;
...
//scanning
float Andor885::startKineticsFTExtTrig(string directory,int num,int newBin, int newGain){
	CheckExists(0)
	first=true;
	waitIdle();
	...
	//num=15000;
	Andor885SelCam(err_val=SetNumberKinetics(num),this)//number of pictures to acquire
	Andor885SelCam(err_val=SetExposureTime(0),this)//delay before exposure
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
		logFile.write(string("Error: could not set exposure time- ")+::toString((int)err_val),DEBUGANDOR885);
		heatUp();
		clickAbort();
	}
	float oldGain=gain;
	...
	gain=oldGain;
	bin=oldBin;
	Andor885SelCam(SetKineticCycleTime(0),this)//we want minimum repition rate
	Andor885SelCam(SetFrameTransferMode(1),this)//frame transfer mode
	//string directory=".\\output\\"+scanName+"\\";
	//directory=".\\output\\";
	//cout<<"directory is "<<directory<<endl;
	//string s=".\\output\\"+scanName+"\\";
	char* dir=toCharStar(directory);
	logFile.write(string("Spooling directory is ")+string(dir),DEBUGANDOR885);
	Andor885SelCam(err_val=SetSpool(1,2,dir,10),this)
	delete[] dir;
	if (err_val!=DRV_SUCCESS){
	...
	float acc,exp2;
	Andor885SelCam(GetAcquisitionTimings(&exp2,&acc,&minTrigTime),this)
		logFile.write("FT Mode Scanning. Number Pictures: "+::toString(num)+" delay: "+::toString(exp2)+" Min Trigger Time (not updated properly by Andor SDK): "+::toString(minTrigTime),DEBUGANDOR885);
	long ret=0;
//	Andor885SelCam(err_val=GetSizeOfCircularBuffer(&ret),this)
/*	if (err_val!=DRV_SUCCESS){
		cout<<"could not get size of circular buffer: "<<err_val<<endl;
		system("pause");
		heatUp();
...
*/
	//Andor885SelCam(err_val=PrepareAcquisition(),this)
...
float Andor885::startInternalFTKinetics(string directory,int num,float exp,int newBin, int newGain){
	CheckExists(0)
	first=true;
	waitIdle();
	//setImageRegion(0,100,100);do this in autofocus
	unsigned int err_val;
	//ResetEvent(this->newImage);
	Andor885SelCam(err_val=SetTriggerMode(6),this)//external start trigger
	Andor885SelCam(err_val=SetAcquisitionMode(3),this)//kinetics
	Andor885SelCam(err_val=SetReadMode(4),this)//image acquisition mode
	
	//num=15000;
	Andor885SelCam(err_val=SetNumberKinetics(num),this)//number of pictures to acquire
	Andor885SelCam(err_val=SetExposureTime(exp),this)//delay before exposure
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
		logFile.write(string("Error: could not set exposure time- ")+::toString((int)err_val),DEBUGANDOR885);
		heatUp();
		clickAbort();
...
//stepwise autofocus
float Andor885::startNonFTKineticsExtTrig(int num,float newExp,int newBin, int newGain){
	CheckExists(0)
	...
	Andor885SelCam(SetSpool(0,2,"",10),this)
	Andor885SelCam(SetReadMode(4),this)//image acquisition mode	
	Andor885SelCam(SetAcquisitionMode(3),this)//kinetics
	Andor885SelCam(SetFrameTransferMode(0),this)//frame transfer mode
	Andor885SelCam(SetTriggerMode(1),this)//external trigger
	Andor885SelCam(SetExposureTime(newExp),this)
	Andor885SelCam(SetNumberKinetics(num),this)//number of pictures to acquire
	//SetKineticCycleTime(0);
	Andor885SelCam(SetFrameTransferMode(0),this)//frame transfer mode
	Andor885SelCam(SetExposureTime(newExp),this)
	float exp2,acc,kin;
	Andor885SelCam(err_val=GetAcquisitionTimings(&exp2,&acc,&kin),this)
	...
	long index;
	Andor885SelCam(GetSizeOfCircularBuffer(&index),this)
...
//autofocus
float Andor885::startFTKinetics(int num,float newExp,int newBin, int newGain){
	CheckExists(0)
	unsigned int err_val;
	waitIdle();
	ResetEvent(this->newImage);
	...
	if (newExp<0.035){
		...
		Andor885SelCam(err_val=SetFrameTransferMode(0),this)//frame transfer mode
	}else {
		Andor885SelCam(err_val=SetFrameTransferMode(1),this)
	}
	if (err_val!=DRV_SUCCESS){
	...
	focusNum=num;
	Andor885SelCam(err_val=SetKineticCycleTime(0),this)
	if (err_val!=DRV_SUCCESS){
	...
	Andor885SelCam(err_val=SetSpool(0,2,"",10),this)
	if (err_val!=DRV_SUCCESS){
		cout<<"could not set spooling options: "<<err_val<<endl;
		system("pause");
		heatUp();
...
CImg<unsigned short>* Andor885::getNewPic(){
	CImg<unsigned short> *copyImage=new CImg<unsigned short>((maxXPixel-minXPixel+1)/bin,(maxYPixel-minYPixel+1)/bin);
	CheckExists(copyImage)
	LARGE_INTEGER start;
	QueryPerformanceCounter(&start);
	...
	unsigned int err_val;
	Andor885SelCam(err_val=GetMostRecentImage16(copyImage->data(),(maxXPixel-minXPixel+1)*(maxYPixel-minYPixel+1)/bin/bin),this)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
		cout<<"could not get 16bit image: "<<err_val<<endl;
		system("pause");
		heatUp();
	...
	int status;
	Andor885SelCam(GetStatus(&status),this)
	if (status!=DRV_IDLE){
...
CImg<unsigned short>* Andor885::getOldest(){
	CImg<unsigned short> *copyImage=new CImg<unsigned short>((maxXPixel-minXPixel+1)/bin,(maxYPixel-minYPixel+1)/bin);
	CheckExists(copyImage)
	unsigned int err_val;
	Andor885SelCam(err_val=GetOldestImage16(copyImage->data(),(maxXPixel-minXPixel+1)*(maxYPixel-minYPixel+1)/bin/bin),this)
	if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
		cout<<"could not get 16bit image: "<<err_val<<endl;
		system("pause");
		heatUp();
...
unsigned __stdcall Andor885::FocusThread(void* param){
	Andor885* cam=(Andor885*) param;
...
//	CImg<unsigned short>* best=new CImg<unsigned short>((cam->maxXPixel-cam->minXPixel+1)/cam->bin,(cam->maxYPixel-cam->minYPixel+1)/cam->bin);
	...
	while(cam->isIdle());//wait for Andor885 to start
	while(cam->focusing && ind<cam->focusNum){
		bool b=false;
		if (!cam->isIdle()){//only wait if Andor885 is not done acquiring
WAITFORIMAGE:			WaitForSingleObject(cam->newImage,INFINITE);
		...
		Andor885SelCam(err_val=GetOldestImage16(image->data(),((cam->maxXPixel-cam->minXPixel+1)/cam->bin)*((cam->maxYPixel-cam->minYPixel+1)/cam->bin)),cam)
		if (DRV_SUCCESS!=err_val){//look in current directory for .ini file
			if (err_val=20024) goto WAITFORIMAGE;
			cout<<"could not get 16bit image: "<<err_val<<endl;
			int stat;
			Andor885SelCam(GetStatus(&stat),cam)
   ...
   Timer threadWait(true);
	...
	logFile.write("Focus wait for score threads took "+::toString(threadWait.getTime())+"ms",DEBUGANDOR885);
	...
	delete [] images;
	logFile.write("Total calculation time is "+::toString(totalTime)+"ms Average is "+::toString(totalTime/cam->focusNum),DEBUGANDOR885);
	...
	focusLogFile.write(Timer::getSysTime()+"\t"+::toString((int)bestInd)+"\t"+::toString(maxScore),DEBUGANDOR885);
	return bestInd;
}
/*

LONGLONG Andor885::getScore(CImg<unsigned short>& image){
...
*/
unsigned __stdcall Andor885::threadGroup(void* param){
...
unsigned __stdcall Andor885::getScore(void* param){//CImg<unsigned short>* Cimage,LONGLONG* sum1){
	Score* s=(Score*) param;
...
void Andor885::getScore(CImg<unsigned short>* Cimage,LONGLONG* sum1){
	(*sum1)=0;
	//CImg<unsigned short>* Cimage=&image;
	//return Cimage->norm();
	//autocorrelation method of autofocus follows:  F=sum(i=1:M-1,j=1:N)g(i,j)*g(i+1,j)-sum(i=1:M-2,j=1:N)g(i,j)*g(i+2,j)
